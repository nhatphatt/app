"""
Test chatbot promotion response
"""
import os
import sys
import asyncio
...
async def test_promotion():
    print("=" * 70)
    print("Testing Chatbot Promotion Response")
    print("=" * 70)
    ...
    # Get menu items with promotions
    print("\n[3] Getting menu items with promotions...")
    ...
    print(f"✓ Items with promotion: {len(promo_items)}")
    
    if promo_items:
        print("\nPromotions found:")
        for item in promo_items:
            original = int(item.get('price', 0))
            discounted = int(item.get('discounted_price', original))
            print(f"   🎉 {item.get('name')}: {discounted:,}đ (giảm từ {original:,}đ)")
    else:
        print("\n⚠️  No promotions found in menu items")
    ...
    print("\n[5] Testing promotion query...")
    print("User message: 'Có món nào đang giảm giá không?'")
    
    response = await response_gen.generate_response(
        intent="ask_promotion",
        entities={},
        context={"store_id": store['id']},  # Add store_id to context
        store_id=store['id'],
        original_message="Có món nào đang giảm giá không?",
